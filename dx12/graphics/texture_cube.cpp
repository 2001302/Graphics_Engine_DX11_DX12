...
TextureCube::TextureCube(const wchar_t *file_name,
    ...
    DirectX::CreateDDSTextureFromFile(GpuDevice::Get().device.Get(), upload,
                                      file_name, &resouce_, false, isCubeMap);
    auto finished = upload.End(GpuDevice::Get().command_queue.Get());
    finished.wait();

    auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(
        resouce_.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
...
void TextureCube::Allocate(DescriptorHeap *heap, UINT &index) {
    ...
    if (!isBrdf_) {
        D3D12_SHADER_RESOURCE_VIEW_DESC desc = {
            resouce_->GetDesc().Format, D3D12_SRV_DIMENSION_TEXTURECUBE,
            D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING};

        desc.TextureCube.MipLevels = resouce_->GetDesc().MipLevels;
        desc.TextureCube.MostDetailedMip = 0;
        desc.TextureCube.ResourceMinLODClamp = 0;

        GpuDevice::Get().device->CreateShaderResourceView(
            resouce_.Get(), &desc, cpu_handle_);
    } else {
        D3D12_SHADER_RESOURCE_VIEW_DESC desc = {
            resouce_->GetDesc().Format, D3D12_SRV_DIMENSION_TEXTURE2D,
            D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING};

        desc.Texture2D.MipLevels = resouce_->GetDesc().MipLevels;
