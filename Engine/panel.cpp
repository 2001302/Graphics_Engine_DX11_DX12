...
void Panel::OnFrame(float deltaTime) {
...

    {
        ...
        // =======================================================================================================
        // TreeNodes and Headers streatch to the entire remaining work area. To
        // put them in nodes what we need to do is to tell ImGui out work area
        // is shorter. We can achieve that right now only by using columns API.
        //
        ...
        ImGui::SameLine();
        ImGui::Dummy(ImVec2(
            35, 0)); //  magic number - Crude & simple way to nudge over the
                     //  output pin. Consider using layout and springs
        ImGui::SameLine();
        ed::BeginPin(uniqueId++, ed::PinKind::Output);
        ImGui::Text("Out ->");
        ed::EndPin();
        ...
        float width =
            135; // bad magic numbers. used to define width of tree widget
        ...
        ImGui::SetColumnWidth(0, width + ImGui::GetStyle().WindowPadding.x +
        ...
        // =====================================================================================
        // Tooltips, combo-boxes, drop-down menus need to use a work-around to
        // place the "overlay window" in the canvas. To do this, we must defer
        // the popup calls until after we're done drawing the node material.
        //
        ...
        ImGui::SameLine();
        ImGui::Dummy(
            ImVec2(85, 0)); // Hacky magic number to space out the output pin.
        ImGui::SameLine();
        ed::BeginPin(uniqueId++, ed::PinKind::Output);
        ImGui::Text("Out ->");
        ed::EndPin();
        ...
        if (ImGui::Button(popup_text)) {
            do_popup =
                true; // Instead of saying OpenPopup() here, we set this bool,
                      // which is used later in the Deferred Pop-up Section
        }
        ed::EndNode();
        if (firstframe) {
        ...
        // This entire section needs to be bounded by Suspend/Resume!  These
        // calls pop us out of "node canvas coordinates" and draw the popups in
        // a reasonable screen location.
        ed::Suspend();
        // There is some stately stuff happening here.  You call "open popup"
        // exactly once, and this causes it to stick open for many frames until
        // the user makes a selection in the popup, or clicks off to dismiss.
        // More importantly, this is done inside Suspend(), so it loads the
        // popup with the correct screen coordinates!
        if (do_popup) {
            ImGui::OpenPopup("popup_button"); // Cause openpopup to stick open.
            do_popup = false; // disable bool so that if we click off the popup,
                              // it doesn't open the next frame.
        }

        // This is the actual popup Gui drawing section.
        if (ImGui::BeginPopup("popup_button")) {
            // Note: if it weren't for the child window, we would have to
            // PushItemWidth() here to avoid a crash!
            ...
            if (ImGui::Button("Option 1")) {
                ...
                ImGui::CloseCurrentPopup(); // These calls revoke the popup open
                                            // state, which was set by OpenPopup
                                            // above.
            }
            ...
            ImGui::EndChild();
            ImGui::EndPopup(); // Note this does not do anything to the popup
                               // open/close state. It just terminates the
                               // content declaration.
        }
        ...
        // Plot Widgets
        // =========================================================================================
        // Note: most of these plots can't be used in nodes missing, because
        // they spawn tooltips automatically, so we can't trap them in our
        // deferred pop-up mechanism.  This causes them to fly into a random
        // screen location.
        ...
        ImVector<LinkInfo>
            m_Links; // List of live links. It is dynamic unless you
                     // want to create read-only view over nodes.
        int m_NextLinkId =
            100; // Counter to help generate link ids. In real application this
                 // will probably based on pointer to user data structure.
    }
