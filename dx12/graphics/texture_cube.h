...
class TextureCube : public GpuBuffer {
  public:
    TextureCube() : GpuBuffer(){};
    TextureCube(const wchar_t *file_name,
                ComPtr<ID3D12GraphicsCommandList> command_list,
                bool isCubeMap, bool isBrdf = false) {

        DirectX::ResourceUploadBatch upload(
            dx12::GpuCore::Instance().device.Get());
        upload.Begin();
        DirectX::CreateDDSTextureFromFile(
            dx12::GpuCore::Instance().device.Get(), upload, file_name, &buffer_,
            false, isCubeMap);
        auto finished =
            upload.End(dx12::GpuCore::Instance().command_queue.Get());
        finished.wait();
