...
class PSO {
  public:
    PSO(const wchar_t *Name)
        : m_Name(Name), m_RootSignature(nullptr), m_PSO(nullptr) {}
...
class GraphicsPSO : public PSO {
  ...
  public:
    ...
    void SetDepthTargetFormat(DXGI_FORMAT DSVFormat, UINT MsaaCount = 1,
                              UINT MsaaQuality = 0);
    ...
    void SetRenderTargetFormats(UINT NumRTVs, const DXGI_FORMAT *RTVFormats,
                                DXGI_FORMAT DSVFormat, UINT MsaaCount = 1,
                                UINT MsaaQuality = 0);
    void SetInputLayout(UINT NumElements,
                        const D3D12_INPUT_ELEMENT_DESC *pInputElementDescs);
    void SetPrimitiveRestart(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBProps);

    // These const_casts shouldn't be necessary, but we need to fix the API to
    // accept "const void* pShaderBytecode"
    void SetVertexShader(const void *Binary, size_t Size) {
        m_PSODesc.VS =
            CD3DX12_SHADER_BYTECODE(const_cast<void *>(Binary), Size);
    }
