...
namespace Engine 
{
	...
	{
	private:
		Direct3D() : vsync_enabled(false), swap_chain(0), device(0), device_context(0), viewport(D3D11_VIEWPORT()) {}
		Direct3D(const Direct3D& x) : vsync_enabled(x.vsync_enabled), swap_chain(x.swap_chain), device(x.device), device_context(x.device_context), viewport(x.viewport){}
