...
class ShadowMappingPSO : public GraphicsPSO {
  public:
    void Initialize() override {
        ...
        texture_range.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 10);
    ...
    void Render(common::Model *world,
                SamplerState *sampler_state,
                ComPtr<ID3D12Resource> global_consts, MeshRenderer *renderer,
        ...
        context->SetViewportAndScissorRect(0, 0, (UINT)depth_buffer->GetWidth(),
        ...
        for (auto mesh : renderer->GetMeshes()) {
            ...
            context->GetList()->SetGraphicsRootConstantBufferView(
                4, renderer->MaterialConsts().Get()->GetGPUVirtualAddress());
...
class SkinnedShadowMappingPSO : public GraphicsPSO {
  public:
    void Initialize() override {
        // rootSignature
        // s0 ~ s6
        ...
        root_parameters[4].InitAsConstantBufferView(
            2, 0, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC,
            D3D12_SHADER_VISIBILITY_ALL);
        root_parameters[5].InitAsDescriptorTable(1, &bone_range,
                                                 D3D12_SHADER_VISIBILITY_ALL);
        ...
        ShaderUtil::CreateVertexShader(
            GpuCore::Instance().GetDevice(), L"../DX12/Shader/DepthOnlyVS.hlsl",
            depthVS,
    ...
    void Render(common::Model *world, SamplerState *sampler_state,
                ComPtr<ID3D12Resource> global_consts, MeshRenderer *renderer,
                DepthBuffer *depth_buffer, Animator *animator) {
