#include "physically_common.hlsli" // 쉐이더에서도 include 사용 가능
...
float3 GetNormal(PixelShaderInput input) {
    ...
    {
        float3 normal =
            normalTex.SampleLevel(linearSampler, input.texcoord, 0.0).rgb;
...
float3 SpecularIBL(float3 albedo, float3 normalWorld, float3 pixelToEye,
    ...
    float3 specularIrradiance =
        specularIBLTex
            .SampleLevel(linearSampler, reflect(-pixelToEye, normalWorld),
                         roughness * 10.0f)
            .rgb;
...
PixelShaderOutput main(PixelShaderInput input) {
    ...
    float3 albedo = useAlbedoMap
                        ? albedoTex.Sample(linearSampler, input.texcoord).rgb
                        : material.albedo;
    float ao = useAOMap
                   ? aoTex.SampleLevel(linearSampler, input.texcoord, 0.0).r
                   : 1.0;
    float metallic = useMetallicMap
                         ? metallicTex.Sample(linearSampler, input.texcoord).b
                         : material.metallic;
    float roughness = useRoughnessMap
                          ? roughnessTex.Sample(linearSampler, input.texcoord).g
                          : material.roughness;
    float3 emission =
        useEmissiveMap ? emissiveTex.Sample(linearSampler, input.texcoord).rgb
                       : float3(0, 0, 0);
    float3 ambientLighting = AmbientLightingByIBL(
        albedo, normalWorld, pixelToEye, ao, metallic, roughness);

    float3 directLighting = float3(0, 0, 0);

    // 포인트 라이트만 먼저 구현
    [unroll] for (int i = NUM_DIR_LIGHTS; i < NUM_DIR_LIGHTS + NUM_POINT_LIGHTS;
                  ++i) {
        ...
        float3 radiance =
            light[i].radiance *
            saturate((light[i].fallOffEnd - length(lightVec)) /
