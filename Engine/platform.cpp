...
bool Platform::OnStart() {
    ...
    RECT wr = {0, 0, Engine::Env::Instance().screen_width,
    ...
    Engine::Env::Instance().main_window =
    ...
    if (!Engine::Env::Instance().main_window) {
        std::cout << "CreateWindow() failed." << std::endl;
        return false;
    }

    ShowWindow(Engine::Env::Instance().main_window, SW_SHOWDEFAULT);
    UpdateWindow(Engine::Env::Instance().main_window);
...
bool Platform::OnStop() {
    // Shutdown the window.
    if (Engine::Env::Instance().full_screen) {
        ChangeDisplaySettings(NULL, 0);
    }

    // Remove the window.
    DestroyWindow(Engine::Env::Instance().main_window);
    Engine::Env::Instance().main_window = NULL;
