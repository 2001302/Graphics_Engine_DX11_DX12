...
static DXGI_FORMAT GetDXGIFormat(const DDS_PIXELFORMAT &ddpf) {
    if (ddpf.flags & DDS_RGB) {
        ...
        case 32:
            ...
            // the the RED/BLUE masks for 10:10:10:2 formats. We assumme below
        ...
        if (SUCCEEDED(hr) && tex != nullptr) {
            ...
            if (arraySize > 1) {
                ...
                SRVDesc.Texture1DArray.MipLevels =
                    (!mipCount) ? -1 : ResourceDesc.MipLevels;
...
CreateTextureFromDDS(_In_ ID3D12Device *d3dDevice,
...

    {
        ...
        std::unique_ptr<D3D12_SUBRESOURCE_DATA[]> initData(
            new D3D12_SUBRESOURCE_DATA[subresourceCount]);
        if (!initData) {
            return E_OUTOFMEMORY;
        }
        ...
        if (SUCCEEDED(hr)) {
            hr = CreateD3DResources(d3dDevice, resDim, twidth, theight, tdepth,
                                    ...
                                    forceSRGB, isCubeMap, initData.get(),
                                    texture, textureView);
        ...
        if (SUCCEEDED(hr)) {
            ...
            // CommandContext::InitializeTexture(DestTexture, subresourceCount,
            // initData.get());
...
_Use_decl_annotations_ HRESULT CreateDDSTextureFromMemory(
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE textureView, TextureLoadResult &result,
...
_Use_decl_annotations_ HRESULT CreateDDSTextureFromFile(
    ...
    // transfer ownership to result -  no need to access, just wait with cleanup
    result.ddsData = move(ddsData);
