...
PixelShaderOutput main(CubeMappingPixelShaderInput input) {
    ...
    if (textureToDraw == 0) {
        output.pixelColor =
            g_envTex.SampleLevel(g_sampler, input.posModel.xyz, mipLevel);
    } else if (textureToDraw == 1) {
        output.pixelColor =
            g_specularTex.SampleLevel(g_sampler, input.posModel.xyz, mipLevel);
    } else {
        output.pixelColor = g_irradianceTex.SampleLevel(
            g_sampler, input.posModel.xyz, mipLevel);
    }
