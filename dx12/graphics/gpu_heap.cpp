...
GpuHeap::GpuHeap(UINT num_descriptors, D3D12_DESCRIPTOR_HEAP_TYPE type,
    ...
    sampleStates.push_back(sampler::linearWrapSS);
    sampleStates.push_back(sampler::linearClampSS);
    sampleStates.push_back(sampler::shadowPointSS);
    sampleStates.push_back(sampler::shadowCompareSS);
    sampleStates.push_back(sampler::pointWrapSS);
    sampleStates.push_back(sampler::linearMirrorSS);
    sampleStates.push_back(sampler::pointClampSS);
    ...
    for (int i = 0; i < sampleStates.size(); ++i) {
        GpuCore::Instance().device->CreateSampler(&sampleStates[i],
                                                  sampler_heap_cpu_handle);
