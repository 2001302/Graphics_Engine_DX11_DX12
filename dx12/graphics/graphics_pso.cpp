...
void GraphicsPSO::SetPrimitiveTopologyType(
...
void GraphicsPSO::SetPrimitiveRestart(
    D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBProps) {
    m_PSODesc.IBStripCutValue = IBProps;
}

void GraphicsPSO::SetDepthTargetFormat(DXGI_FORMAT DSVFormat, UINT MsaaCount,
                                       UINT MsaaQuality) {
    SetRenderTargetFormats(0, nullptr, DSVFormat, MsaaCount, MsaaQuality);
}

void GraphicsPSO::SetRenderTargetFormat(DXGI_FORMAT RTVFormat,
                                        DXGI_FORMAT DSVFormat, UINT MsaaCount,
                                        UINT MsaaQuality) {
    SetRenderTargetFormats(1, &RTVFormat, DSVFormat, MsaaCount, MsaaQuality);
}

void GraphicsPSO::SetRenderTargetFormats(UINT NumRTVs,
...
//    HashCode = Utility::HashState(m_InputLayouts.get(),
//                                  m_PSODesc.InputLayout.NumElements, HashCode);
...
//
//        // Reserve space so the next inquiry will find that someone got here
//        // first.
...
//
//    if (firstCompile) {
//        assert_SUCCEEDED(g_Device->CreateComputePipelineState(
//            &m_PSODesc, MY_IID_PPV_ARGS(&m_PSO)));
