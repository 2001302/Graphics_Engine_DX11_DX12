...
bool Direct3D::InitMainScene(int screenWidth, int screenHeight)
{
...

	{
		...
		swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)backBufferPtr.GetAddressOf());
		// Create the render target view with the back buffer pointer.
		device->CreateRenderTargetView(backBufferPtr.Get(), NULL, &render_target_view);
...

	{
		...
		device->CreateTexture2D(&depth_buffer_desc, NULL, depth_stencil_buffer.GetAddressOf());
...

	{
		...
		device->CreateDepthStencilView(depth_stencil_buffer.Get(), &depth_stencil_view_desc, &depth_stencil_view);

	}

	// Bind the render target view and depth stencil buffer to the output render pipeline.
	device_context->OMSetRenderTargets(1, &render_target_view, depth_stencil_view.Get());

	{
		...
		device->CreateRasterizerState(&raster_desc, solid_rasterizer_state.GetAddressOf());
...

	{
		...
		device->CreateRasterizerState(&rast_desc_wire,
			wire_rasterizer_state.GetAddressOf());
...
void Direct3D::BeginScene(float red, float green, float blue, float alpha)
{
	...
	device_context->ClearDepthStencilView(depth_stencil_view.Get(), D3D11_CLEAR_DEPTH, 1.0f, 0);
